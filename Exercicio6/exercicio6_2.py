...
def serie_no_espaco_param(label_serie, df_serie, label_espaco_param, df_espaco_param,
                          nome_arquivo_estatisticas_espaco_param, is_plotar_kmeans):
    # parâmetros fixos
    ...
    df_espaco_param[['variancia_ao_quadrado', 'curtose', coluna_espaco_param]].to_csv(nome_arq_dummy, index=False)
    k_means_e_metodo_do_cotovelo(nome_arq_dummy, k_array, metodos_do_cotovelo, is_plotar_kmeans)
    shutil.move('./k_2.png', './{}_k_2.png'.format(label_espaco_param))

    # ... (com a série inclída)
    df_serie_espaco_param = pd.concat(
        [df_serie[['variancia_ao_quadrado', 'curtose', coluna_espaco_param]], df_espaco_param])
...
if __name__ == '__main__':
    ...
    nome_arquivo_espaco_param_ESPB = './files_6_1_sem_noise/espaco_param_ESPB.csv'
    nome_arquivo_espaco_param_EDF = './files_6_1_sem_noise/espaco_param_EDF.csv'
    ...
    df_covid_ndc_eua = df_covid_completo[['iso_code', 'date', new_cases]]

    is_df_covid_eua = (df_covid_ndc_eua['iso_code'] == country_iso_code)
    df_covid_eua = df_covid_ndc_eua[is_df_covid_eua]
    arr_covid_ndc_eua = df_covid_eua[new_cases].values
    ...
    serie_no_espaco_param(label_serie, df_covid_ndc_eua, 'espb', df_espaco_param_ESPB,
                          nome_arquivo_estatisticas_covid_ndc_espb, is_plotar_kmeans)
    serie_no_espaco_param(label_serie, df_covid_ndc_eua, 'edf', df_espaco_param_EDF,
                          nome_arquivo_estatisticas_covid_ndc_edf, is_plotar_kmeans)
    ...
    serie_no_espaco_param(label_serie, df_surf_temp, 'espb', df_espaco_param_ESPB,
                          nome_arquivo_estatisticas_surf_temp_espb, is_plotar_kmeans)
    serie_no_espaco_param(label_serie, df_surf_temp, 'edf', df_espaco_param_EDF,
                          nome_arquivo_estatisticas_surf_temp_edf, is_plotar_kmeans)
